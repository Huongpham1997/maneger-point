		...
		if (!empty($_POST['submit_students'])) {
			...
			if($resultStudents['success']){ // đã vào đến đây là thêm thành công 
				...
				if($resultList['success']){
					...
					$this->view('students/students-form',['resultMessaqugeAdd' => 'Thêm thành công vui lòng chuyển sang trang danh sách để kiểm tra!']);
				}
			}else{
		...
		if(!empty($_GET['id'])){
			...
			if (!empty($_POST['submit_students'])) {
				...
				if($resultStudents['success']){
					$this->view('students/students-index', ['resultMessageAdd' => $resultStudents['message']]);
				}else{
					$this->view('students/students-form',['resultMessageAdd' => $resultStudents['message']]);
				}
			}else{
