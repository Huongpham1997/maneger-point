...
<div class="container">
    ...
    <?php if(!empty($data['data'])){ 
        ...
        ?>
        <form action="?url=subject/update&id=<?= $row['id'] ?>&class_id=<?= $_GET['class_id'] ?>" method="post">
    ...
    <?php }}else{ ?>
        <form action="?url=subject/add&class_id=<?= $data['class_id'] ?>" method="post">
            <input class="form-control" required name="subject_title" type="text" placeholder="Tên môn học" value=""><br>
            <input type="hidden" name="class_id" value="<?= $data['class_id'] ?>">
            <input class="btn btn-success" type="submit" name="submit_add_subject" value="Thêm mới môn học">
        </form>
    <?php } ?>
